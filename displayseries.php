    ...
    <div id="content">
        <div class="container">
            <div id="list" class="row">
                ...
                while ($pageno <= $maxpageno && $pageno < 26) {
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL, 'http://eurostar.mix.tm/stalker_portal/server/load.php?type=vod&action=get_ordered_list&category=1074&&genre=186&p='.$pageno.'&sortby=addedd&JsHttpRequest=1-xml');
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
                    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                        'User-Agent: Mozilla/5.0 (QtEmbedded; U; Linux; C) AppleWebKit/533.3 (KHTML, like Gecko) MAG200 stbapp ver: 2 rev: 250 Safari/533.3',
